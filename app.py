...
with right:
    ...
    if uploaded_file is not None and display_image is not None:
        ...
        info = DISEASE_INFO.get(
            ...
            {
                "title": nice_label,
                "summary": "The model detected this disease class from the uploaded leaf image.",
                "symptoms": ["Visible disease-related leaf changes detected by the model."],
                "prevention": ["Maintain crop hygiene", "Inspect plants regularly", "Consult a local expert if symptoms spread"],
                "action": "Monitor the crop carefully and consult local agricultural guidance for exact treatment."
            }
        )

        st.markdown("<div class='result-box'>", unsafe_allow_html=True)
        st.markdown(
            f"<div class='result-label'>Detected: {info['title']}</div>",
            unsafe_allow_html=True,
        )
        st.markdown(
