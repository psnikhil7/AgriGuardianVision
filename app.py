...
MODEL_PATH = "healthy_disease_model.keras"
MODEL_ZIP = "healthy_model.zip"
CLASS_FILE = "class_names_binary.txt"
...
def ensure_model_exists():
    ...
    for root, dirs, files in os.walk("."):
        for file in files:
            if file == "healthy_disease_model.keras":
...
st.markdown(
    ...
    <div class="hero">
        ...
        <p>AI-powered healthy vs diseased plant leaf detection with a clean and reliable professional interface.</p>
    </div>
    """,
    unsafe_allow_html=True,
)
...
with right:
    ...
    if uploaded_file is not None and display_image is not None:
        ...
        for i in top2_idx:
            st.write(f"• {class_names[i].title()} — {float(prediction[0][i])*100:.2f}%")
    else:
