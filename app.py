...
with right:
    ...
    if uploaded_file is not None and display_image is not None:
        ...
        if label == "healthy":
            ...
            st.markdown("<div class='lux-list'>The uploaded leaf appears healthy according to the trained AI model.</div>", unsafe_allow_html=True)

            st.markdown("<div class='minor-title'>Recommended Action</div>", unsafe_allow_html=True)
            st.markdown(
                "<div class='lux-list'>Continue routine monitoring, proper watering, balanced nutrients, and good crop hygiene.</div>",
                unsafe_allow_html=True,
            )
